...
function greatestProduct()
  ...
  for i in 1:rows
    for j in 1:cols
	    # can we look right?  (covers left case)
	    if j <= cols - 3
	    ...
	    # can we look down?  (covers up case)
	    if i <= rows - 3
	    ...
	    # can we look down-right?  (covers up-left)
	    ...
	    # can we look down-left?  (covers up-right)
	    if i <= rows-3 && j >= 4
	      p = grid[i,j] * grid[i+1,j-1] * grid[i+2,j-2] * grid[i+3,j-3]
		    if p > gp
		      gp = p
...
@time @test greatestProduct() == 70600674